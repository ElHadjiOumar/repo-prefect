...
with bronze_data  as (
  ...
  CAST(json_extract_scalar(_airbyte_data, '$.main.humidity') AS DOUBLE) AS main_humidity,
  CAST(json_extract_scalar(_airbyte_data, '$.main.pressure') AS DOUBLE) AS main_pressure,
  CAST(json_extract_scalar(_airbyte_data, '$.wind.speed') AS DOUBLE) AS wind_speed,
  CAST(json_extract_scalar(_airbyte_data, '$.clouds.all') AS DOUBLE) AS clouds,
  ...
  AND _airbyte_emitted_at >= '{{ run_started_at }}' -- Pour les données nouvelles
